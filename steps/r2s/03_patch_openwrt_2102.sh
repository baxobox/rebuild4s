...
#enable cpuinfo in proc/cpunfo and OF mode for pcie, support hwmon
cp $ROOTDIR/patches/999-display-model-name-in-proc-cpuinfo.patch target/linux/generic/hack-5.4/
cp $ROOTDIR/patches/106-arm64-rockchip-add-OF-node-for-pcie-eth-on-NanoPi-R4S.patch target/linux/rockchip/patches-5.4/
cp $ROOTDIR/patches/008-rockchip-add-hwmon-support-for-SoCs-and-GPUs.patch target/linux/rockchip/patches-5.4/

#--------------------add luci-app-diskman------------
cp -R $ROOTDIR/package/parted package/
cp -R $ROOTDIR/package/luci-app-diskman package/
